...
void userProcessColorImageFunc_laser(bgr *ptrImage) {
	...
	case 2:
		// seeing orange
		// Orange Ball: H [0:3,240:255], S [191:222], V [233:255]
		specs_h = 250;
		specs_hrad = 10;
		if((specs_h-specs_hrad)<0) // wrap 0->360
		{
	...
	if (ptrImage != NULL) {
		...
		for (r=0;r<IMAGE_ROWS-CEILING_OFFSET;r++) {
...
//            }
		...
		if (new_vision_data == 0)
		{
			switch(toggle) {
			case 0:
				blue_rbar = (int) (final_object_stats[largest_object].center_r - IMAGE_ROWS / 2);
				blue_cbar = (int) (final_object_stats[largest_object].center_c - IMAGE_COLUMNS / 2);
				blueNumPixels = (int) final_object_stats[largest_object].num_pixels_in_object;
				toggle++;
				break;
			case 1:
				green_rbar = (int) (final_object_stats[largest_object].center_r - IMAGE_ROWS / 2);
				green_cbar = (int) (final_object_stats[largest_object].center_c - IMAGE_COLUMNS / 2);
				greenNumPixels = (int) final_object_stats[largest_object].num_pixels_in_object;
				toggle++;
			case 2:
				orange_rbar = (int) (final_object_stats[largest_object].center_r - IMAGE_ROWS / 2);
				orange_cbar = (int) (final_object_stats[largest_object].center_c - IMAGE_COLUMNS / 2);
				orangeNumPixels = (int) final_object_stats[largest_object].num_pixels_in_object;
				toggle = 0;
				break;
			}
